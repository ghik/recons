...
object ReCon extends ProjectGroup("recon") {
  ...
  override def commonSettings: Seq[Def.Setting[?]] = Seq(
    ...
    ideBasePackages := Seq("com.github.ghik.recon"),
    Compile / doc / sources := Nil,

    projectInfo := ModuleInfo(
      nameFormal = "ReCon",
      description = "Scala Debug Shell",
      homepage = Some(url("https://github.com/ghik/recon")),
      startYear = Some(2024),
      licenses = Vector(
      ...
      scmInfo = Some(ScmInfo(
        browseUrl = url("https://github.com/ghik/recon.git"),
        connection = "scm:git:git@github.com:ghik/recon.git",
        devConnection = Some("scm:git:git@github.com:ghik/recon.git"),
      )),
      developers = Vector(
  ...
  lazy val client = mkSubProject
    ...
    .settings(
      ...
      Universal / packageName := s"recon-client_${scalaVersion.value}-${version.value}",
